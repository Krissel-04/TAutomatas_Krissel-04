...
void cerraduraPositiva(char alfabeto[][maxLongitudCadenas], int numElementos, int maxLongitud, char *cadena) {
    int longitudCadena = 1 + rand() % maxLongitud;
    cadena[0] = '\0'; 
...
int main() {
    ...
    printf("Introduce los elementos del alfabeto (separados por espacios):\n");
    ...
    while (totalGeneradas < maxCadenas) {
        ...
        cerraduraPositiva(alfabeto, numElementosAlfabeto, maxLongitud, cadena);
        if (!existeCadena(cadenasGeneradas, totalGeneradas, cadena)) {
            strcpy(cadenasGeneradas[totalGeneradas], cadena);
            totalGeneradas++;
        }
    }
    printf("\nCadenas generadas usando la cerradura positiva:\n{ ");
